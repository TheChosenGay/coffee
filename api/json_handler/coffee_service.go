...
func (s *JsonCoffeeServiceHandler) MakeJsonServiceHandler() {
	// list coffees
	http.HandleFunc("/coffee/list", func(w http.ResponseWriter, r *http.Request) {
		reqId := rand.Int64N(1000000)
		ctx := context.WithValue(r.Context(), "requestId", reqId)
		// call real service
		if err := s.listCoffees(ctx, w, r); err != nil {
			api.WriteToJson(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		}
	})

	// get coffee by id
	http.HandleFunc("/coffee/get", func(w http.ResponseWriter, r *http.Request) {
		reqId := rand.Int64N(1000000)
		ctx := context.WithValue(r.Context(), "requestId", reqId)
		// call real service
		if err := s.getCoffeeById(ctx, w, r); err != nil {
			api.WriteToJson(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
			return
		}
	})
...
func (s *JsonCoffeeServiceHandler) listCoffees(ctx context.Context, w http.ResponseWriter, _ *http.Request) error {
...
func (s *JsonCoffeeServiceHandler) getCoffeeById(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
